    ...
    */
    ...
    public boolean perform(AbstractBuild build, Launcher launcher, BuildListener listener) {
        ...
        try {
            ...
            if ("static".equals(apiType)) {
                listener.getLogger()
                        .println("The vAPI query string input chosen is static. jSON input is: '" + vAPIInput + "'");
                jSonInput = vAPIInput;
            } else {
                if (vJsonInputFile == null || vJsonInputFile.trim().equals("")) {
                    listener.getLogger()
                            .println(
                                    "The vAPI query string chosen is dynamic. jSON input file dynamic workspace directory: '"
                                            + build.getWorkspace() + "'");
                } else {
                    listener.getLogger()
                            .println("The vAPI query string chosen is dynamic. jSON input file static location: '"
                                    + vJsonInputFile.trim() + "'");
                }
                jSonInput = utils.loadJSONFromFile(
                        build.getId(),
                        build.getNumber(),
                        "" + build.getWorkspace(),
                        vJsonInputFile,
            ...
            // ----------------------------------------------------------------------------------------------------------------
            String output = utils.executeAPI(
                    jSonInput,
                    apiUrl,
                    vAPIUrl,
                    authRequired,
            ...
            if (!"success".equals(output)) {
                listener.getLogger()
                        .println("Failed to call vAPI for build " + build.getId() + " " + build.getNumber() + "\n");
    ...
    public static final class DescriptorImpl extends BuildStepDescriptor<Builder> {
        ...
        public FormValidation doCheckVAPIUrl(@QueryParameter String value) throws IOException, ServletException {
            if (value.length() == 0) return FormValidation.error("Please set the Verisium Manager vAPI HOST ");
        ...
        public FormValidation doCheckApiUrl(@QueryParameter String value) throws IOException, ServletException {
            if (value.length() == 0) return FormValidation.error("Please set the Verisium Manager vAPI URL ");
            if (value.length() < 8) return FormValidation.warning("Isn't the name too short?");
            return FormValidation.ok();
        }
