...
public class VMGRLaunchStepImpl extends SynchronousNonBlockingStepExecution implements Serializable {
    ...
    protected Void run() throws Exception {
        ...
        if (step.isPipelineNodes()) {
            ...
            listener.getLogger().println("Master workspace location: " + workspace);
        }

        // Check if workspace is there, unless create it
        File theWSDir = new File(workspace);
        // if the directory does not exist, create it
        if (!theWSDir.exists()) {
            listener.getLogger().println("Creating workspace directory: " + theWSDir.getName());
            if (!theWSDir.mkdir()) {
                listener.getLogger().println("Failed to create workspace directory: " + theWSDir.getAbsolutePath());
                throw new IOException("Failed to create workspace directory: " + theWSDir.getAbsolutePath());
            } else {
                listener.getLogger().println("Workspace directory created");
            }
        }
        ...
        if (step.isAdvConfig()) {
            ...
            listener.getLogger().println("The connection timeout is: 1 minute");
            listener.getLogger().println("The read api timeout is: 30 minutes");
        }

        listener.getLogger()
                .println("In case build is interrupted, session will get paused: "
                        + step.isPauseSessionOnBuildInterruption());
        ...
        if ("hybrid".equals(step.getExecutionType())) {
            ...
            if (step.isEnvVarible()) {
                listener.getLogger().println("An environment variable file was selected.");
            }
            if (step.isAttrValues()) {
                listener.getLogger().println("An attribute values file was selected.");
            }
            if (step.isDefineVarible()) {
                listener.getLogger().println("A define variable file was selected.");
            }

            if (step.isUseUserOnFarm()) {
                listener.getLogger().println("A User's Credential use was selected.");
                ...
                if ("dynamic".equals(step.getUserFarmType())) {
                    ...
                    listener.getLogger()
                            .println("The credential file was set to be deleted after use: "
                                    + step.isDeleteCredentialInputFile());
                }
                if (step.getEnvSourceInputFile() != null
                        && !"".equals(step.getEnvSourceInputFile().trim())) {
        ...
        if (step.isWaitTillSessionEnds()) {
            ...
            listener.getLogger()
                    .println("In case session is at state \'inaccessible\' the build will "
                            + step.getInaccessibleResolver());
            listener.getLogger()
                    .println("In case session is at state \'failed\' the build will " + step.getFailedResolver());
            listener.getLogger()
                    .println("In case session is at state \'stopped\' the build will " + step.getStoppedResolver());
            listener.getLogger()
                    .println("In case session is at state \'suspended\' the build will " + step.getSuspendedResolver());
            listener.getLogger()
                    .println("In case session is at state \'done\' the build will " + step.getDoneResolver());
            ...
            listener.getLogger()
                    .println("User choosed to mark regression as Failed in case all runs are failing: "
                            + step.isMarkBuildAsFailedIfAllRunFailed());
            listener.getLogger()
                    .println("User choosed to fail the job in case all runs are failing: "
                            + step.isFailJobIfAllRunFailed());
            listener.getLogger()
                    .println("User choosed to mark regression as Passed in case all runs are passed: "
                            + step.isMarkBuildAsPassedIfAllRunPassed());
            listener.getLogger()
                    .println("User choosed to fail the job unless all runs are passed: "
            ...
            if (step.isGenerateJUnitXML()) {
                ...
                jUnitRequestHolder = new JUnitRequestHolder(
                        step.isGenerateJUnitXML(),
                        step.isExtraAttributesForFailures(),
                        step.getStaticAttributeList(),
                        step.isNoAppendSeed());
                listener.getLogger()
                        .println("Extra Attributes in JUnit Report: " + step.isExtraAttributesForFailures());
                if (step.isExtraAttributesForFailures()) {
                    listener.getLogger()
                            .println("Extra Attributes list in JUnit Report is: " + step.getStaticAttributeList());
                }
            }

            stepHolder = new StepHolder(
                    step.getInaccessibleResolver(),
                    step.getStoppedResolver(),
                    step.getFailedResolver(),
                    step.getDoneResolver(),
                    step.getSuspendedResolver(),
                    step.isWaitTillSessionEnds(),
                    step.getStepSessionTimeout(),
                    jUnitRequestHolder,
                    step.isMarkBuildAsFailedIfAllRunFailed(),
                    step.isFailJobIfAllRunFailed(),
                    step.isMarkBuildAsPassedIfAllRunPassed(),
        ...
        if (step.isVMGRBuildArchive()) {
            ...
            listener.getLogger()
                    .println("Delete also session directory on disk: " + step.isDeleteAlsoSessionDirectory());
            listener.getLogger()
                    .println("Use dedicated credentials for deleting the session: "
                            + step.isGenericCredentialForSessionDelete());
            ...
            vMGRBuildArchiver = new VMGRBuildArchiver(
                    step.isVMGRBuildArchive(),
                    step.isGenericCredentialForSessionDelete(),
                    step.isGenericCredentialForSessionDelete(),
                    step.getArchiveUser(),
                    step.getArchivePassword(),
                    step.getFamMode(),
                    step.getFamModeLocation());
        }
        ...
        try {
            ...
            if ("batch".equals(step.getExecutionType())) {
                if (step.getSessionsInputFile() == null
                        || step.getSessionsInputFile().trim().equals("")) {
                    listener.getLogger()
                    ...
                    listener.getLogger()
                            .println("The session input file chosen is static. Sessions input file name is: '"
                                    + step.getSessionsInputFile().trim() + "'");
                }
                ...
                sessionNames = utils.loadDataFromInputFiles(
                        ...
                        buildNumber,
                        "" + workspace,
                        step.getSessionsInputFile(),
                        listener,
                        step.isDeleteSessionInputFile(),
                        "session names",
                        "sessions.input");
                ...
                // first
                // BatchExecManager batchExecManager = new BatchExecManager(listener, step.getExecutionScript(),
                // step.getExecutionShellLocation(), step.getExecutionVsifFile(),buildId, buildNumber);
                // batchExecManager.execBatchCommand(filePath);
                utils.batchExecManager(
                        listener,
                        step.getExecutionScript(),
                        step.getExecutionShellLocation(),
                        step.getExecutionVsifFile(),
                        buildId,
                        buildNumber,
                        launcher);
                sessionNames = utils.loadDataFromInputFiles(
                        buildId, buildNumber, "" + workspace, "", listener, false, "session names", "sessions.input");
                if (sessionNames.length == 0) {
                    listener.getLogger().println("No session were found within sessions.input file.  Exit Job.\n");
                    throw new Exception(
                            "Failed to launch sessions for build " + buildId + " " + buildNumber + "\n"); // false;
                }
                ...
                if ("static".equals(step.getVsifType())) {
                    listener.getLogger()
                            .println("The VSIF file chosen is static. VSIF file static location is: '"
                                    + step.getVSIFName() + "'");
                    vsifFileNames = new String[1];
                    vsifFileNames[0] = step.getVSIFName();
                } else {
                    if (step.getVSIFInputFile() == null
                            || step.getVSIFInputFile().trim().equals("")) {
                        // listener.getLogger().println("The VSIF file chosen is dynamic. VSIF directory dynamic
                        // workspace directory: '" + build.getWorkspace() + "'");
                    } else {
                        listener.getLogger()
                                .println("The VSIF file chosen is static. VSIF file name is: '"
                                        + step.getVSIFInputFile().trim() + "'");
                    }
                    ...
                    vsifFileNames = utils.loadDataFromInputFiles(
                            ...
                            buildNumber,
                            "" + workspace,
                            step.getVSIFInputFile(),
                            listener,
                            step.isDeleteInputFile(),
                            "VSIF",
                            "vsif.input");
                }
                ...
                if (step.isEnvVarible()) {
                    if (step.getEnvVaribleFile() == null
                            || step.getEnvVaribleFile().trim().equals("")) {
                        listener.getLogger()
                                .println(
                        ...
                        listener.getLogger()
                                .println("The environment varible file chosen is static. Environment file name is: '"
                                        + step.getEnvVaribleFile().trim() + "'");
                    }
                    jsonEnvInput = utils.loadJSONEnvInput(
                            buildId, buildNumber, "" + workspace, step.getEnvVaribleFile(), listener);
                ...
                if (step.isAttrValues()) {
                    if (step.getAttrValuesFile() == null
                            || step.getAttrValuesFile().trim().equals("")) {
                        listener.getLogger()
                                .println(
                                        "The attribute values file chosen is dynamic. Attribute values File directory dynamic workspace directory: '"
                                                + workspace + "'");
                    } else {
                        listener.getLogger()
                                .println("The attribute values file chosen is static. Attribute values file name is: '"
                                        + step.getAttrValuesFile().trim() + "'");
                    }
                    jsonAttrValuesInput = utils.loadJSONAttrValuesInput(
                            buildId, buildNumber, "" + workspace, step.getAttrValuesFile(), listener);
                    listener.getLogger()
                ...
                if (step.isDefineVarible()) {
                    if (step.getDefineVaribleFile() == null
                            || step.getDefineVaribleFile().trim().equals("")) {
                        listener.getLogger()
                                .println(
                                        "The define values file chosen is dynamic. Define values File directory dynamic workspace directory: '"
                                                + workspace + "'");
                    } else {
                        listener.getLogger()
                                .println("The define values file chosen is static. Define values file name is: '"
                                        + step.getDefineVaribleFile().trim() + "'");
                    }
                    jsonDefineInput = utils.loadJSONDefineInput(
                            buildId, buildNumber, "" + workspace, step.getDefineVaribleFile(), listener);
                    listener.getLogger().println("Found the following define values for the vsif: " + jsonDefineInput);
                }

                if ("dynamic".equals(step.getUserFarmType())) {
                    if (step.getCredentialInputFile() == null
                            || step.getCredentialInputFile().trim().equals("")) {
                        listener.getLogger()
                                .println(
                        ...
                        listener.getLogger()
                                .println("The credential file chosen is static. Credential file name is: '"
                                        + step.getCredentialInputFile().trim() + "'");
                    }
                    farmUserPassword = utils.loadFileCredentials(
                            buildId,
                            buildNumber,
                            "" + workspace,
                            step.getCredentialInputFile(),
                            listener,
                            step.isDeleteCredentialInputFile());
            ...
            String output = utils.executeVSIFLaunch(
                    ...
                    step.isAdvConfig(),
                    jsonEnvInput,
                    step.isUseUserOnFarm(),
                    step.getUserFarmType(),
                    farmUserPassword,
                    stepHolder,
                    step.getEnvSourceInputFile(),
                    workingJobDir,
                    vMGRBuildArchiver,
                    step.isUserPrivateSSHKey(),
                    jsonAttrValuesInput,
                    tmpExecutionType,
                    sessionNames,
                    step.getEnvSourceInputFileType(),
                    launcher,
                    jsonDefineInput);
