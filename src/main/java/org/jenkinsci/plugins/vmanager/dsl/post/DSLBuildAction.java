...
public class DSLBuildAction extends PostActionBase
    ...
    public List<String> getJobSessions() {
        ...
        String ids = BuildStatusMap.getValue(
                vmgrRun.getRun().getId(), vmgrRun.getRun().getNumber(), vmgrRun.getJobWorkingDir() + "", "id", true);
    ...
    public String getvManagerLink(boolean buildLevel) {
        ...
        if (this.build != null) {
            ...
            VMGRRun tmpVmgrRun =
                    new VMGRRun(this.build, workingDir, job.getBuildDir().getAbsolutePath());
            String id = BuildStatusMap.getValue(
                    tmpVmgrRun.getRun().getId(),
                    tmpVmgrRun.getRun().getNumber(),
                    tmpVmgrRun.getJobWorkingDir() + "",
                    "id",
                    true);
            ...
            if (id.indexOf(",") > 0) {
                ...
                String vAPIUrl = BuildStatusMap.getValue(
                        tmpVmgrRun.getRun().getId(),
                        tmpVmgrRun.getRun().getNumber(),
                        tmpVmgrRun.getJobWorkingDir() + "",
                        "url",
